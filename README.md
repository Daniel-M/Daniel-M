...
As a software engineer I'm proud of
 - Creating and maintaining an scheduled multi step web-scraping data ingestion pipeline (NodeJS+Elastisearch+Redis) that included fine-tuned DeepLearning and MachineLearning clustering models(Golang/Python+Pytorch) for clothing classification for a fashiontech startup during the 2019-2021 period. I did other things but this is what I'm most proud of
 - A short project for a fintech that wanted to migrate an algorithm from one language to another, found what seemed a bug, turned out to be a flaw in their algorithm itself (Python).
 - A GraphQL backend for IoT and a RESTful data ingestion API (Golang/NodeJS+InfluxDB+MongoDB), with small memory fingerprint, during 2021-2022 period. Later migrated this backend to go and it was 10x performant, lower fingerprint and faster response times
